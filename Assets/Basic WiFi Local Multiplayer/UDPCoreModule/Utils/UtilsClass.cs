...
using System.Globalization;
	...
	public static Quaternion StringToQuaternion(string _rot)
	{

		Quaternion  newRot;
		string[] newString = Regex.Split(_rot,";");
		if(_rot.ToLower().Contains(","))
		 {
		   CultureInfo ci = (CultureInfo)CultureInfo.CurrentCulture.Clone();
            ci.NumberFormat.CurrencyDecimalSeparator = ",";
			
		newRot = new Quaternion ( float.Parse(newString[0],NumberStyles.Any,ci), float.Parse(newString[1],NumberStyles.Any,ci)
		,float.Parse(newString[2],NumberStyles.Any,ci),float.Parse(newString[3],NumberStyles.Any,ci));
		...
		else
		{
		  CultureInfo ci2 = (CultureInfo)CultureInfo.CurrentCulture.Clone();
			...
			newRot= new Quaternion ( float.Parse(newString[0],NumberStyles.Any,ci2), float.Parse(newString[1],NumberStyles.Any,ci2)
		,float.Parse(newString[2],NumberStyles.Any,ci2),float.Parse(newString[3],NumberStyles.Any,ci2));
	...
	public static string QuaternionToString(Quaternion _rot)
	{
		return _rot.x+";"+_rot.y+";"+_rot.z+";"+_rot.w;
	}
