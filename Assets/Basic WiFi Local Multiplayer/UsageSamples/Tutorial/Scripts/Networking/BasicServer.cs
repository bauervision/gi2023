...
public class BasicServer : MonoBehaviour
{
    ...
    public UnityEvent onClient1Connected = new UnityEvent();
    public UnityEvent onClient2Connected = new UnityEvent();
        ...
        try
        {
            if (!connectedClients.ContainsKey(data.pack[1]))
            {
                ...
                if (connectedClients.Count == 1)
                    onClient1Connected.Invoke();

                if (connectedClients.Count == 2)
                    onClient2Connected.Invoke();
