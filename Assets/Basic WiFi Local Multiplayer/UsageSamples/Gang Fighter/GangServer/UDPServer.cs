...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	 ...
	 try
	{
		if (!connectedClients.ContainsKey (data.pack [1])) {
			...
			client.character_index = int.Parse(data.pack [4]);
	...
	void EmitResponse(Client client, IPEndPoint playerIP )
	{
		...
		//store  player info in msg field
		send_pack ["msg"] = client.id + ":" + client.name + ":" + client.position+":"+client.character_index;
	...
	void EmitPlayersToPlayer(Client client, IPEndPoint playerIP)
	{
		...
		foreach (KeyValuePair<string, Client> entry in connectedClients) {
			...
			if (entry.Value.id != client.id) {
				...
				send_pack ["msg"] = entry.Value.id + ":" + entry.Value.name + ":" + entry.Value.position+":"+entry.Value.character_index;
	 ...
	 try
	{
		if (connectedClients.ContainsKey (data.pack [1])) {
			...
			connectedClients [data.pack [1]].character_index = int.Parse(data.pack [3]);

			IPEndPoint playerIP = data.anyIP;

			Client player = connectedClients [data.pack [1]];

			EmitRespawnPlayer(playerID,connectedClients [data.pack [1]].name,data.pack [4],
			connectedClients [data.pack [1]].character_index.ToString(),
			 playerIP );
 ...
 /// <summary>
	...
	/// </remarks>
	void  EmitRespawnPlayer(string playerID,string name,string _position,string avatar, IPEndPoint playerIP )
	{
		...
		Dictionary<string, string> send_pack = new Dictionary<string, string> ();
		 ...
		 send_pack ["msg"] = playerID + ":" + name +":"+_position+ ":" + avatar;
	      ...
	      var target = connectedClients [targetID];
		   ...
		   // if the target has not lost all of its health
		     if(target.health - target.damage > 0)
			 {
			   ...
			   target.health -=target.damage; //health decrement
			 }
