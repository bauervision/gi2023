    ...
    public class RayfireRigidRootEditor : Editor
    {
        ...
        void UI_Activation()
        {
            ...
            if (exp_act == true)
            {
                ...
                EditorGUI.BeginChangeCheck();
                root.activation.off = EditorGUILayout.Slider (gui_act_ofs, root.activation.off, 0, 10f);
                if (EditorGUI.EndChangeCheck() == true)
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.activation.off = root.activation.off;
                        SetDirty (scr);
                    }
                ...
                if (root.activation.off > 0)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    root.activation.loc = EditorGUILayout.Toggle (gui_act_loc, root.activation.loc);
                    if (EditorGUI.EndChangeCheck())
                        foreach (RayfireRigidRoot scr in targets)
                        {
                            scr.activation.loc = root.activation.loc;
                            SetDirty (scr);
                        }
                ...
                EditorGUI.BeginChangeCheck();
                root.activation.vel = EditorGUILayout.Slider (gui_act_vel, root.activation.vel, 0, 5f);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.activation.vel = root.activation.vel;
                        SetDirty (scr);
                    }
                ...
                */
                ...
                EditorGUI.BeginChangeCheck();
                root.activation.act = EditorGUILayout.Toggle (gui_act_act, root.activation.act);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.activation.act = root.activation.act;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.activation.imp = EditorGUILayout.Toggle (gui_act_imp, root.activation.imp);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.activation.imp = root.activation.imp;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.activation.con = EditorGUILayout.Toggle (gui_act_con, root.activation.con);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.activation.con = root.activation.con;
                        SetDirty (scr);
                    }
                ...
                if (root.activation.con == true)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    root.activation.uny = EditorGUILayout.Toggle (gui_act_uny, root.activation.uny);
                    if (EditorGUI.EndChangeCheck())
                        foreach (RayfireRigidRoot scr in targets)
                        {
                            scr.activation.uny = root.activation.uny;
                            SetDirty (scr);
                        }
                    ...
                    EditorGUI.BeginChangeCheck();
                    root.activation.atb = EditorGUILayout.Toggle (gui_act_acd, root.activation.atb);
                    if (EditorGUI.EndChangeCheck())
                        foreach (RayfireRigidRoot scr in targets)
                        {
                            scr.activation.atb = root.activation.atb;
                            SetDirty (scr);
                        }
                ...
                if (root.activation.l == true)
                {
                    ...
                    EditorGUI.BeginChangeCheck();
                    root.activation.lay = EditorGUILayout.LayerField (gui_act_lay, root.activation.lay);
                    if (EditorGUI.EndChangeCheck())
                        foreach (RayfireRigidRoot scr in targets)
                        {
                            scr.activation.lay = root.activation.lay;
                            SetDirty (scr);
                        }
                }
        ...
        void UI_Limitations()
        {
            ...
            if (exp_lim == true)
            {
                ...
                EditorGUI.BeginChangeCheck();
                root.demolition.limitations.col = EditorGUILayout.Toggle (gui_lim_col, root.demolition.limitations.col);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.demolition.limitations.col = root.demolition.limitations.col;
                        SetDirty (scr);
                    }
                ...
                {
                    EditorGUI.BeginChangeCheck();
                    root.demolition.limitations.sol = EditorGUILayout.Slider (gui_lim_sol, root.demolition.limitations.sol, 0, 10f);
                    if (EditorGUI.EndChangeCheck() == true)
                        foreach (RayfireRigidRoot scr in targets)
                        {
                            scr.demolition.limitations.sol = root.demolition.limitations.sol;
                            SetDirty (scr);
                        }
                ...
                EditorGUI.BeginChangeCheck();
                root.demolition.limitations.vis = EditorGUILayout.Toggle (gui_lim_vis, root.demolition.limitations.vis);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.demolition.limitations.vis = root.demolition.limitations.vis;
                        SetDirty (scr);
                    }
                ...
                EditorGUI.BeginChangeCheck();
                root.demolition.limitations.bld = EditorGUILayout.Toggle (gui_lim_slc, root.demolition.limitations.bld);
                if (EditorGUI.EndChangeCheck())
                    foreach (RayfireRigidRoot scr in targets)
                    {
                        scr.demolition.limitations.bld = root.demolition.limitations.bld;
                        SetDirty (scr);
                    }
