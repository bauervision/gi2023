...
namespace Gaia
{
    ...
    public class FreeCamera : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (!m_inputCaptured)
            {
                if (!holdRightMouseCapture && ControlFreak2.CF2Input.GetMouseButtonDown(0))
                    CaptureInput();
                else if (holdRightMouseCapture && ControlFreak2.CF2Input.GetMouseButtonDown(1))
                    CaptureInput();
            }
            ...
            if (m_inputCaptured)
            {
                ...
                {
                    sprintSpeed += (ControlFreak2.CF2Input.GetAxis(m_scrollWheelKey) * m_speedIncreaseValue);
                ...
                if (!holdRightMouseCapture && ControlFreak2.CF2Input.GetKeyDown(KeyCode.Escape))
                    ReleaseInput();
                else if (holdRightMouseCapture && ControlFreak2.CF2Input.GetMouseButtonUp(1))
                    ReleaseInput();
            }

            float rotStrafe = ControlFreak2.CF2Input.GetAxis(m_mouseXKey);
            float rotFwd = ControlFreak2.CF2Input.GetAxis(m_mouseYKey);
            ...
            float speed = Time.deltaTime * (ControlFreak2.CF2Input.GetKey(KeyCode.LeftShift) ? sprintSpeed : moveSpeed);
            float forward = speed * ControlFreak2.CF2Input.GetAxis(m_verticalKey);
            float right = speed * ControlFreak2.CF2Input.GetAxis(m_horizontalKey);
            float up = speed * ((ControlFreak2.CF2Input.GetKey(KeyCode.E) ? 1f : 0f) - (ControlFreak2.CF2Input.GetKey(KeyCode.Q) ? 1f : 0f));
